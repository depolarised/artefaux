...
Artefaux v2 contains **85 test records** (derived from publicly available sources) plus **70 paired clean parent records** (from open-access PhysioNet datasets). v2 is deliberately weighted toward clinically-unusable ("discard") recordings. The corpus is organized by corruption mode, not by clinical condition.
...
| Real-noise SNR ladder | 30 | PTB-XL 500 Hz base + NSTDB noise | EM, MA, BW at {−6, −4, −2, 0, +2} dB SNR (aggressive low ladder) | Yes (PTB-XL) |
...
| Engineering extremes: v2 extreme | 18 | PTB-XL parents + deterministic corruption | High-amplitude & whole-precordium saturation, multi-lead integrity failures (flat/stuck/NaN/lead-off/dropout), harder electrode faults, gross baseline excursions, compound "wild" multi-mode | Yes (PTB-XL) |

**Total records: 85**  
**Paired clean parents: 70**
...
**Real-noise pairs:** PTB-XL records selected deterministically (deduplicated by patient, seeded random sample) and paired with NSTDB noise segments, resampled to 500 Hz and mixed according to the aggressive-low SNR ladder (−6, −4, −2, 0, +2 dB) — v2 drops the clean end to concentrate on the accept/reject decision region. Each pair's `rhythm_class` is an *authored/representative* label (the intended rhythm spread), **not** read from the selected record.
...
- **v2.0.0 (2026-07-16):** Discard/extreme reweighting. Corpus grows to 85 records (15 / 30 / 40); the real-noise SNR ladder moves to an aggressive-low `{−6, −4, −2, 0, +2}` dB sweep concentrated on the accept/reject decision region; +18 extreme engineering cases (whole-precordium saturation, multi-lead integrity failures, harder electrode faults, compound "wild" records). Adds a full-10-second-per-lead (`strip8`) waveform review pack and a colour-coded explainer. Existing engineering ids `001–022` and the 15 naturally-poor ids are unchanged.
